...
from .EAEmodel import AmpereEAEModel
...
EAEBatch_fields = ['batch_doc_id', 'batch_wnd_id', 'batch_tokens', 'batch_text', 'batch_piece_idxs', 'batch_token_start_idxs', 
                   'batch_trigger', 'batch_arguments', 'batch_input', 'batch_target', 'batch_amrgraph']
...
def EAE_collate_fn(batch):
    return EAEBatch(
        ...
        batch_amrgraph=[instance["amrgraph"] for instance in batch], 
    )
...
class AmpereEAETrainer(BasicTrainer):
    ...
    def load_model(self, checkpoint=None):
        if checkpoint:
            ...
            self.model = AmpereEAEModel(self.config, self.tokenizer, self.type_set)
    ...
    def process_data(self, data, amr_file):
        assert self.tokenizer, "Please load model and tokneizer before processing data!"
        
        with open(amr_file) as fp:
            lines = fp.readlines()
        amr_objs = [json.loads(l) for l in lines]
        
        amr_map = defaultdict(dict)
        for amr_obj in amr_objs:
            amr_map[amr_obj["doc_id"]][amr_obj["wnd_id"]] = amr_obj["amrgraph"]
        ...
        logger.info(f"Generate {len(new_data)} Ampere EAE instances from {n_total} EAE instances")

        return new_data

    def train(self, train_data, dev_data, **kwargs):
        self.load_model()
        internal_train_data = self.process_data(train_data, kwargs["train_amr"])
        internal_dev_data = self.process_data(dev_data, kwargs["dev_amr"])
    ...
    def predict(self, data, **kwargs):
        ...
        internal_data = self.process_data(data, kwargs["test_amr"])
