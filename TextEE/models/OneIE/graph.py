...
class Graph(object):
    ...
    def to_dict(self):
        ...
        entities = [[i, j, entity_itos[k]] for (i, j, k), (_, _, l), s in zip(self.entities, self.mentions, self.entity_scores)]
        triggers = [[i, j, trigger_itos[k]] for (i, j, k), l in zip(self.triggers, self.trigger_scores)]
        relations = [[i, j, relation_itos[k]] for (i, j, k), l in zip(self.relations, self.relation_scores)]
        roles = [[i, j, role_itos[k], l] for (i, j, k), l in zip(self.roles, self.role_scores)]
        # roles = []
        # for (tri_idx, ent_idx, role_type) in self.roles:
        #     try:
        #         tri = triggers[tri_idx]
        #         ent = entities[ent_idx]
        #         role_t = role_itos[role_type]
        #         roles.append([list(tri), list(ent), role_t])
        #     except:
        #         ipdb.set_trace()
