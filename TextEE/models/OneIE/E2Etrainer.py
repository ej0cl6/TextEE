...
from .E2Emodel import OneIEE2EModel
...
class OneIEE2ETrainer(BasicTrainer):
    ...
    def load_model_(self, checkpoint=None):
        ...
        if checkpoint:
            ...
            self.model = OneIEE2EModel(self.config, self.vocabs, self.valid_patterns)
    ...
    def train(self, train_data, dev_data, **kwargs):
        ...
        train_set = IEDataset(train_data, self.tokenizer,
        ...
        dev_set = IEDataset(dev_data, self.tokenizer,
                            max_length=self.config.max_length,
                            gpu=True,
                            relation_mask_self=self.config.relation_mask_self,
                            relation_directional=self.config.relation_directional,
    ...
    def predict(self, data, **kwargs):
        ...
        eval_set = IEDataset(data, self.tokenizer,
                             max_length=self.config.max_length,
                             gpu=True,
                             relation_mask_self=self.config.relation_mask_self,
                             relation_directional=self.config.relation_directional,
        ...
        for dt, pred_graph in zip(eval_set, eval_pred_graphs):
            pred_entities = [(e[0], e[1]) for e in pred_graph.entities]
            pred_triggers = [(t[0], t[1], event_type_itos[t[2]]) for t in pred_graph.triggers]
