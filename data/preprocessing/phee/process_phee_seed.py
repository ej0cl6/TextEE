...
def get_split(objs, split):
    ...
    doc_keys = [obj["id"] for obj in objs]
    all_idxs = np.arange(len(doc_keys))
    np.random.shuffle(all_idxs)
    n_test = int(len(doc_keys)*0.2)
    train_doc_ids = [doc_keys[i] for i in all_idxs[:-2*n_test]]
    dev_doc_ids = [doc_keys[i] for i in all_idxs[-2*n_test:-n_test]]
    test_doc_ids = [doc_keys[i] for i in all_idxs[-n_test:]]
    
    with open(os.path.join(split, "train.txt"), "w") as fp:
        for doc_id in sorted(train_doc_ids):
            fp.write(doc_id+"\n")
    with open(os.path.join(split, "dev.txt"), "w") as fp:
        for doc_id in sorted(dev_doc_ids):
            fp.write(doc_id+"\n")
    with open(os.path.join(split, "test.txt"), "w") as fp:
